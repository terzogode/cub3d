...
/*   Updated: 2026/02/11 23:27:37 by mbrighi          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	check_collision(t_ray *ray, t_game *g)
{
	...
	if (g->map->grid[ray->map_y][ray->map_x] == '1'
		|| g->map->grid[ray->map_y][ray->map_x] == 'B')
	{
		ray->hit = 1;
	}
	else if (g->map->grid[ray->map_y][ray->map_x] == 'D')
	{
		i = 0;
		while (i < g->door_count)
		{
			if (g->doors[i].x == ray->map_x && g->doors[i].y == ray->map_y)
			{
